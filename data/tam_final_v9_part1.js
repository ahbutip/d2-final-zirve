window.examsData = window.examsData || {};

window.examsData['tam_kapsamli_final_v9'] = window.examsData['tam_kapsamli_final_v9'] || {
    "title": "Zirve Deneme Sınavı 6 (Ustalık Seviyesi)",
    "description": "Antigravity tarafından Cem Kopuz (9), Ufuk Çorumlu (9), Sezer Akçer (8) ve Ömer Can Kızılay (8) anatomi dağılımına sadık kalınarak oluşturulmuş Ustalık Seviyesi Zirve Denemesi.",
    "questions": []
};

const v9Part1 = [
    // --- ANATOMİ (34 Soru) ---

    // 1. Prof. Dr. Ufuk Çorumlu
    {
        "id": "v9_anat_1",
        "subject": "Anatomi",
        "question": "Mitral kapak darlığı olan bir hastada büyüyen sol atriyum (basis cordis), hemen arkasındaki hangi yapıya bası yaparak yutma güçlüğüne (disfaji) neden olur?",
        "options": [
            "A) Trachea",
            "B) Aorta ascendens",
            "C) Truncus pulmonalis",
            "D) Esophagus",
            "E) Vena cava superior"
        ],
        "answer": "D) Esophagus",
        "correctAnswer": 3,
        "explanation": "Kalbin arka yüzü (basis cordis) esas olarak sol atriyumdan oluşur ve arkasında doğrudan esophagus (yemek borusu) ile komşudur."
    },
    {
        "id": "v9_anat_2",
        "subject": "Anatomi",
        "question": "Dirsek ön yüzünde aponeurosis bicipitalis'in (lacertus fibrosus) hemen altında yer alan ve damar yolu açılırken korunması gereken arter hangisidir?",
        "options": [
            "A) A. ulnaris",
            "B) A. radialis",
            "C) A. brachialis",
            "D) A. axillaris",
            "E) A. profunda brachii"
        ],
        "answer": "C) A. brachialis",
        "correctAnswer": 2,
        "explanation": "Fossa cubiti'de N. medianus ile birlikte bulunan A. brachialis, V. mediana cubiti'den kan alınırken aponeuroz sayesinde korunur."
    },
    {
        "id": "v9_anat_3",
        "subject": "Anatomi",
        "question": "Göz çukurunun (orbita) medial duvarı (lamina papyracea) ile komşu olan paranasal sinüs hangisidir?",
        "options": [
            "A) Sinus maxillaris",
            "B) Sinus frontalis",
            "C) Sinus sphenoidalis",
            "D) Sinus ethmoidalis",
            "E) Cellulae mastoideae"
        ],
        "answer": "D) Sinus ethmoidalis",
        "correctAnswer": 3,
        "explanation": "Sinus ethmoidalis, orbitanın medial duvarı ile komşudur."
    },
    {
        "id": "v9_anat_4",
        "subject": "Anatomi",
        "question": "Üst çene (maxilla) molar ve premolar diş kökleri tabanıyla komşuluk yapan en büyük paranasal sinüs hangisidir?",
        "options": [
            "A) Sinus frontalis",
            "B) Sinus ethmoidalis",
            "C) Sinus maxillaris",
            "D) Sinus sphenoidalis",
            "E) Concha nasalis media"
        ],
        "answer": "C) Sinus maxillaris",
        "correctAnswer": 2,
        "explanation": "Sinus maxillaris'in tabanı, üst çene arka dişlerinin kökleri ile komşudur."
    },
    {
        "id": "v9_anat_5",
        "subject": "Anatomi",
        "question": "T8 vertebra seviyesinde diyaframdan geçen ve Vena cava inferior'un içinden geçtiği deliğin ismi nedir?",
        "options": [
            "A) Hiatus oesophageus",
            "B) Hiatus aorticus",
            "C) Foramen venae cavae",
            "D) Trigonum sternocostale",
            "E) Trigonum lumbocostale"
        ],
        "answer": "C) Foramen venae cavae",
        "correctAnswer": 2,
        "explanation": "T8 seviyesinde centrum tendineum üzerinde bulunan ve V. cava inferior'un geçtiği delik Foramen venae cavae'dir."
    },
    {
        "id": "v9_anat_6",
        "subject": "Anatomi",
        "question": "Mide arka duvarında perforasyon (delinme) olan bir hastada, mide içeriği bursa omentalis'e akmıştır. Bu durumda mide içeriği komşuluğu sebebiyle ilk olarak hangi organa temas eder?",
        "options": [
            "A) Karaciğer",
            "B) Pankreas gövdesi",
            "C) Dalak",
            "D) Sol böbrek alt kutbu",
            "E) Safra kesesi"
        ],
        "answer": "B) Pankreas gövdesi",
        "correctAnswer": 1,
        "explanation": "Midenin hemen arkasında bursa omentalis aracılığıyla pankreas gövdesi bulunur."
    },
    {
        "id": "v9_anat_7",
        "subject": "Anatomi",
        "question": "Mide kanaması şüphesiyle endoskopi yapılan hastada, duodenumun pars superior (1. kıta) arka duvarında kanayan bir ülser saptanıyor. Bu bölgedeki kanamaya komşuluğundan dolayı büyük ihtimalle hangi arter neden olmaktadır?",
        "options": [
            "A) A. mesenterica superior",
            "B) A. splenica",
            "C) A. gastroduodenalis",
            "D) A. hepatica propria",
            "E) A. gastrica sinistra"
        ],
        "answer": "C) A. gastroduodenalis",
        "correctAnswer": 2,
        "explanation": "Duodenum 1. kıtasının hemen arkasından A. gastroduodenalis geçer ve ülserlerde kanama riski yüksektir."
    },
    {
        "id": "v9_anat_8",
        "subject": "Anatomi",
        "question": "Pankreas başı kanseri olan bir hastada sarılık (tıkanma sarılığı) geliştiği gözlemleniyor. Bu durum pankreas başının hemen komşuluğunda bulunan hangi kanalın tümör tarafından sıkıştırılmasından kaynaklanır?",
        "options": [
            "A) Ductus pancreaticus",
            "B) Ductus hepaticus dexter",
            "C) Ductus cysticus",
            "D) Ductus choledochus (Koledok)",
            "E) Ductus submandibularis"
        ],
        "answer": "D) Ductus choledochus (Koledok)",
        "correctAnswer": 3,
        "explanation": "Koledok kanalı pankreas başının içinden veya arkasından geçer, tümörlerde sıkışarak sarılık yapar."
    },
    {
        "id": "v9_anat_9",
        "subject": "Anatomi",
        "question": "Submandibular tükürük bezi taşı olan bir hastada, Wharton kanalının ağız tabanındaki seyri sırasında hemen altından çapraz yaparak geçen sinir hasar görebilir. Bu komşu sinir hangisidir?",
        "options": [
            "A) N. facialis",
            "B) N. hypoglossus",
            "C) N. lingualis",
            "D) N. glossopharyngeus",
            "E) N. vagus"
        ],
        "answer": "C) N. lingualis",
        "correctAnswer": 2,
        "explanation": "N. lingualis, ağız tabanında ductus submandibularis'in altından çapraz yaparak geçer."
    },

    // 10. Prof. Dr. Cem Kopuz
    {
        "id": "v9_anat_10",
        "subject": "Anatomi",
        "question": "Sempatik sinir sisteminin temel fizyolojik işlevleri göz önüne alındığında aşağıdakilerden hangisi beklenmez?",
        "options": [
            "A) Pupillanın genişlemesi",
            "B) Bronşların genişlemesi",
            "C) Tükürük salgısının koyulaşması",
            "D) Gastrointestinal sistem motilitesinde artış",
            "E) Ter bezlerinin uyarılması"
        ],
        "answer": "D) Gastrointestinal sistem motilitesinde artış",
        "correctAnswer": 3,
        "explanation": "Sempatik sistem gastrointestinal motiliteyi azaltırken, parasempatik sistem GİS motilitesini artırır."
    },
    {
        "id": "v9_anat_11",
        "subject": "Anatomi",
        "question": "Gözyaşı bezinin (Gl. lacrimalis) salgı yapmasını sağlayan parasempatik uyarının geçtiği ve otonom işlev gören gangliyon aşağıdakilerden hangisidir?",
        "options": [
            "A) Ganglion ciliare",
            "B) Ganglion oticum",
            "C) Ganglion submandibulare",
            "D) Ganglion pterygopalatinum",
            "E) Ganglion cervicale superius"
        ],
        "answer": "D) Ganglion pterygopalatinum",
        "correctAnswer": 3,
        "explanation": "Gözyaşı bezinin parasempatik inervasyonu N. facialis üzerinden Ganglion pterygopalatinum aracılığıyla sağlanır."
    },
    {
        "id": "v9_anat_12",
        "subject": "Anatomi",
        "question": "Kalbin ileti sisteminde uyarının ventriküllere geçişini yaklaşık 0.1 saniye geciktirerek atriyumların tamamen boşalmasını sağlayan yapı hangisidir?",
        "options": [
            "A) SA nod",
            "B) AV nod",
            "C) His huzmesi",
            "D) Purkinje lifleri",
            "E) Moderator band"
        ],
        "answer": "B) AV nod",
        "correctAnswer": 1,
        "explanation": "AV nod, uyarıyı kısa bir süre geciktirerek atriyum sistolünün tamamlanmasına olanak tanır."
    },
    {
        "id": "v9_anat_13",
        "subject": "Anatomi",
        "question": "Bacağın ön kompartımanında A. tibialis anterior ile birlikte seyreden sinir aşağıdakilerden hangisidir?",
        "options": [
            "A) N. tibialis",
            "B) N. fibularis profundus",
            "C) N. suralis",
            "D) N. saphenus",
            "E) N. ischiadicus"
        ],
        "answer": "B) N. fibularis profundus",
        "correctAnswer": 1,
        "explanation": "Bacağın ön kompartımanında A. tibialis anterior ile N. fibularis profundus beraber seyreder."
    },
    {
        "id": "v9_anat_14",
        "subject": "Anatomi",
        "question": "Bacağın arka kompartımanında ve canalis tarsi içinde A. tibialis posterior ile birlikte seyreden sinir hangisidir?",
        "options": [
            "A) N. suralis",
            "B) N. ischiadicus",
            "C) N. tibialis",
            "D) N. fibularis profundus",
            "E) N. saphenus"
        ],
        "answer": "C) N. tibialis",
        "correctAnswer": 2,
        "explanation": "Bacağın arkasında A. tibialis posterior ve N. tibialis her zaman beraberdir."
    },
    {
        "id": "v9_anat_15",
        "subject": "Anatomi",
        "question": "Larynx kıkırdaklarından hangisi tam bir halka (yüzük) şeklinde olup solunum yolunun tek tam kıkırdak halkasını oluşturur?",
        "options": [
            "A) Cartilago thyroidea",
            "B) Cartilago cricoidea",
            "C) Cartilago epiglottica",
            "D) Cartilago arytenoidea",
            "E) Cartilago corniculata"
        ],
        "answer": "B) Cartilago cricoidea",
        "correctAnswer": 1,
        "explanation": "Cartilago cricoidea, yüzük şeklinde olup solunum yolunun tek tam kıkırdak halkasıdır."
    },
    {
        "id": "v9_anat_16",
        "subject": "Anatomi",
        "question": "Adem elması olarak da bilinen 'prominentia laryngea' yapısını oluşturan en büyük larynx kıkırdağı hangisidir?",
        "options": [
            "A) Cartilago cricoidea",
            "B) Cartilago arytenoidea",
            "C) Cartilago cuneiformis",
            "D) Cartilago thyroidea",
            "E) Cartilago epiglottica"
        ],
        "answer": "D) Cartilago thyroidea",
        "correctAnswer": 3,
        "explanation": "Cartilago thyroidea, önde prominentia laryngea (Adem elması) çıkıntısını oluşturur."
    },
    {
        "id": "v9_anat_17",
        "subject": "Anatomi",
        "question": "Karın ön duvarı kaslarından biri olan ve alt kenarındaki aponevrozu katlanarak kasık bağını (ligamentum inguinale) oluşturan kas hangisidir?",
        "options": [
            "A) M. rectus abdominis",
            "B) M. transversus abdominis",
            "C) M. obliquus internus abdominis",
            "D) M. obliquus externus abdominis",
            "E) M. pyramidalis"
        ],
        "answer": "D) M. obliquus externus abdominis",
        "correctAnswer": 3,
        "explanation": "Ligamentum inguinale, M. obliquus externus abdominis aponevrozunun alt kenarının katlanmasıyla oluşur."
    },
    {
        "id": "v9_anat_18",
        "subject": "Anatomi",
        "question": "Yüzün mimik kaslarını uyaran N. facialis, motor işlevini yerine getirebilmek için hangi büyük tükürük bezinin içinden geçerek dallarına ayrılır?",
        "options": [
            "A) Glandula sublingualis",
            "B) Glandula submandibularis",
            "C) Glandula parotidea",
            "D) Glandula thyroidea",
            "E) Glandula lacrimalis"
        ],
        "answer": "C) Glandula parotidea",
        "correctAnswer": 2,
        "explanation": "N. facialis, glandula parotidea içinden geçerek yüzün mimik kaslarına dallar verir."
    },

    // 19. Prof. Dr. Sezer Akçer
    {
        "id": "v9_anat_19",
        "subject": "Anatomi",
        "question": "Beyin omurilik sıvısının (BOS) büyük bir kısmını üreten yapı aşağıdakilerden hangisidir?",
        "options": [
            "A) Granulationes arachnoideae",
            "B) Plexus choroideus",
            "C) Ependim hücreleri",
            "D) Sinus sagittalis superior",
            "E) Pia mater"
        ],
        "answer": "B) Plexus choroideus",
        "correctAnswer": 1,
        "explanation": "BOS'un %70'i, ventriküllerin içinde yer alan plexus choroideus'lar tarafından üretilir."
    },
    {
        "id": "v9_anat_20",
        "subject": "Anatomi",
        "question": "Lateral ventriküller ile üçüncü ventrikül arasındaki bağlantıyı sağlayan ve BOS'un geçiş yaptığı delik aşağıdakilerden hangisidir?",
        "options": [
            "A) Foramen Magendie",
            "B) Foramen Luschka",
            "C) Aqueductus mesencephali",
            "D) Foramen interventriculare (Monro)",
            "E) Canalis centralis"
        ],
        "answer": "D) Foramen interventriculare (Monro)",
        "correctAnswer": 3,
        "explanation": "Foramen interventriculare (Monro deliği), lateral ventriküller ile 3. ventrikülü birbirine bağlar."
    },
    {
        "id": "v9_anat_21",
        "subject": "Anatomi",
        "question": "Fetüste oksijenasyonu en yüksek (yaklaşık %80) kanı taşıyan damar aşağıdakilerden hangisidir?",
        "options": [
            "A) Aorta",
            "B) Vena cava inferior",
            "C) Arteria umbilicalis",
            "D) Vena umbilicalis",
            "E) Truncus pulmonalis"
        ],
        "answer": "D) Vena umbilicalis",
        "correctAnswer": 3,
        "explanation": "Plasentadan gelen temiz kanı taşıyan tek ven V. umbilicalis'tir."
    },
    {
        "id": "v9_anat_22",
        "subject": "Anatomi",
        "question": "Vücudun %75'inin lenfini toplayan ve sol angulus venosus'a dökülen ana lenfatik kanal hangisidir?",
        "options": [
            "A) Ductus lymphaticus dexter",
            "B) Cisterna chyli",
            "C) Truncus jugularis sinister",
            "D) Truncus subclavius",
            "E) Ductus thoracicus"
        ],
        "answer": "E) Ductus thoracicus",
        "correctAnswer": 4,
        "explanation": "Ductus thoracicus; vücudun sol yarısı ile karın ve bacakların tamamının lenfini toplayan en büyük kanaldır."
    },
    {
        "id": "v9_anat_23",
        "subject": "Anatomi",
        "question": "Sağ akciğer anatomik olarak fissura obliqua ve fissura horizontalis aracılığıyla kaç loba ayrılır?",
        "options": [
            "A) 1",
            "B) 2",
            "C) 3",
            "D) 4",
            "E) 5"
        ],
        "answer": "C) 3",
        "correctAnswer": 2,
        "explanation": "Sağ akciğer; superior, medius ve inferior olmak üzere 3 loba ayrılır."
    },
    {
        "id": "v9_anat_24",
        "subject": "Anatomi",
        "question": "Sol akciğerde kalbin yerleşimi nedeniyle oluşan incisura cardiaca'nın hemen altında yer alan, sağ akciğerdeki orta lobun (lobus medius) fonksiyonel karşılığı olan dil şeklindeki segment uzantısı hangisidir?",
        "options": [
            "A) Cupula pleurae",
            "B) Lingula pulmonis",
            "C) Apex pulmonis",
            "D) Segmentum basale",
            "E) Carina tracheae"
        ],
        "answer": "B) Lingula pulmonis",
        "correctAnswer": 1,
        "explanation": "Sol akciğer üst lobun alt kısmındaki dil şeklindeki yapıya lingula pulmonis denir."
    },
    {
        "id": "v9_anat_25",
        "subject": "Anatomi",
        "question": "Cerrahi operasyonlarda (rezeksiyon) temel alınan ve damar/safra dağılımına göre belirlenen Couinaud sınıflandırmasında karaciğer kaç fonksiyonel segmente ayrılır?",
        "options": [
            "A) 4",
            "B) 6",
            "C) 8",
            "D) 10",
            "E) 12"
        ],
        "answer": "C) 8",
        "correctAnswer": 2,
        "explanation": "Couinaud sınıflandırmasına göre karaciğer cerrahi olarak 8 bağımsız segmente ayrılır."
    },
    {
        "id": "v9_anat_26",
        "subject": "Anatomi",
        "question": "Karaciğerin sağ ve sol fizyolojik loblarını birbirinden ayıran, üzerinden Vena media hepatica'nın geçtiği hayali çizgiye ne ad verilir?",
        "options": [
            "A) Linea alba",
            "B) Cantlie çizgisi",
            "C) Linea semilunaris",
            "D) Douglas çizgisi",
            "E) Treitz çizgisi"
        ],
        "answer": "B) Cantlie çizgisi",
        "correctAnswer": 1,
        "explanation": "Karaciğeri sağ ve sol iki fizyolojik loba ayıran ana hatta Cantlie çizgisi denir."
    },

    // 27. Prof. Dr. Ömer Can Kızılay
    {
        "id": "v9_anat_27",
        "subject": "Anatomi",
        "question": "Acil servise sağ kol ve yüzünde felç, konuşamama şikayetiyle getirilen bir hastada beynin lateral yüzünü besleyen hangi arterin tıkanmış olması en olasıdır?",
        "options": [
            "A) A. cerebri anterior",
            "B) A. cerebri media",
            "C) A. cerebri posterior",
            "D) A. basilaris",
            "E) A. vertebralis"
        ],
        "answer": "B) A. cerebri media",
        "correctAnswer": 1,
        "explanation": "A. cerebri media (MCA), beynin lateral yüzünün büyük kısmını sular ve Broca alanı ile yüz/kol motor alanlarını besler."
    },
    {
        "id": "v9_anat_28",
        "subject": "Anatomi",
        "question": "Görme alanı kaybı şikayetiyle başvuran bir hastada, oksipital lobu kanlandıran hangi damarın sulama alanında bir tıkanıklık düşünülür?",
        "options": [
            "A) A. cerebri anterior",
            "B) A. cerebri media",
            "C) A. cerebri posterior",
            "D) A. communicans anterior",
            "E) A. ophthalmica"
        ],
        "answer": "C) A. cerebri posterior",
        "correctAnswer": 2,
        "explanation": "A. cerebri posterior (PCA), oksipital lobu (görme merkezi) kanlandırır."
    },
    {
        "id": "v9_anat_29",
        "subject": "Anatomi",
        "question": "Yüksek sese maruz kaldığında rahatsızlık hisseden (hiperakuzi) bir hastada, aşağıdaki kaslardan hangisinin felci düşünülmelidir?",
        "options": [
            "A) M. tensor tympani",
            "B) M. stapedius",
            "C) M. levator veli palatini",
            "D) M. stylopharyngeus",
            "E) M. palatoglossus"
        ],
        "answer": "B) M. stapedius",
        "correctAnswer": 1,
        "explanation": "M. stapedius felcinde yüksek sese karşı koruyucu refleks kaybolduğu için hiperakuzi görülür."
    },
    {
        "id": "v9_anat_30",
        "subject": "Anatomi",
        "question": "Klinikte korkusuzluk ve hiperseksüalite belirtileri (Kluver-Bucy sendromu) gösteren bir hastada, aşağıdaki limbik sistem yapılarından hangisinin bilateral hasarı beklenir?",
        "options": [
            "A) Hipokampüs",
            "B) Amigdala",
            "C) Epifiz bezi",
            "D) Corpus callosum",
            "E) Nucleus caudatus"
        ],
        "answer": "B) Amigdala",
        "correctAnswer": 1,
        "explanation": "Amigdala; korku ve öfke tepkileriyle ilgilidir. Lezyonunda Kluver-Bucy sendromu gelişir."
    },
    {
        "id": "v9_anat_31",
        "subject": "Anatomi",
        "question": "Göğsünden bıçaklanan bir hastada boyun venlerinde dolgunluk, hipotansiyon ve derinden gelen kalp sesleri (Beck Triadı) saptanmıştır. Bu durum, kanın fibröz perikardın esnememesi nedeniyle nerede birikmesinden kaynaklanır?",
        "options": [
            "A) Plevra boşluğu",
            "B) Cavitas pericardiaca",
            "C) Sağ atriyum",
            "D) Fossa ovalis",
            "E) Mediastinum superius"
        ],
        "answer": "B) Cavitas pericardiaca",
        "correctAnswer": 1,
        "explanation": "Bu tablo Kalp Tamponadı'dır. Kan, cavitas pericardiaca'da (perikard boşluğunda) birikir."
    },
    {
        "id": "v9_anat_32",
        "subject": "Anatomi",
        "question": "Sol böbrek karsinomu olan bir hastada sol testiste varikosel (venöz genişleme) saptanmıştır. Bunun temel anatomik nedeni nedir?",
        "options": [
            "A) V. testicularis dextra'nın doğrudan VCI'ye açılması",
            "B) V. testicularis sinistra'nın V. renalis sinistra'ya dökülmesi",
            "C) V. saphena magna yetmezliği",
            "D) Nodi lymphatici lumbales'in tıkanması",
            "E) Ductus deferens kompresyonu"
        ],
        "answer": "B) V. testicularis sinistra'nın V. renalis sinistra'ya dökülmesi",
        "correctAnswer": 1,
        "explanation": "Sol testicular ven, sol renal vene dökülür; sol böbrek tümörleri bu veni tıkayarak varikosel yapar."
    },
    {
        "id": "v9_anat_33",
        "subject": "Anatomi",
        "question": "Kasık bölgesinde şişlik şikayetiyle ameliyata alınan bir hastada, fıtık kesesinin A. epigastrica inferior'un medialinden çıktığı ve Hesselbach üçgeninden geçtiği görülüyor. Bu hastadaki fıtık tipi aşağıdakilerden hangisidir?",
        "options": [
            "A) İndirekt inguinal herni",
            "B) Direkt inguinal herni",
            "C) Femoral herni",
            "D) Umbilikal herni",
            "E) İnsizyonel herni"
        ],
        "answer": "B) Direkt inguinal herni",
        "correctAnswer": 1,
        "explanation": "Hesselbach üçgeninden ve A. epigastrica inferior'un medialinden direkt olarak çıkan fıtıklar direkt inguinal hernilerdir."
    },
    {
        "id": "v9_anat_34",
        "subject": "Anatomi",
        "question": "İnce bağırsağın son kısmında (terminal ileum) emilimi bozulan bir hastada, B12 vitamini eksikliğine bağlı anemi ve safra tuzu emilim bozukluğu geliştiği görülüyor. Hastada etkilenen bağırsak bölümü neresidir?",
        "options": [
            "A) Duodenum",
            "B) Jejunum",
            "C) İleum",
            "D) Çekum",
            "E) Kolon"
        ],
        "answer": "C) İleum",
        "correctAnswer": 2,
        "explanation": "B12 vitamini ve safra tuzları ince bağırsağın son kısmı olan terminal ileumdan emilir."
    },
    // --- FİZYOLOJİ (29 Soru) ---
    // (Ali Koray, Esra Tekin, Hatice Solak, Ayşegül Kuş dağılımında klinik ve temel mekanizmalar)

    {
        "id": "v9_fiz_1",
        "subject": "Fizyoloji",
        "question": "30 yaşında kadın hasta, polikliniğe sinirlilik, aşırı terleme, çarpıntı, kilo kaybı ve gözlerde öne doğru fırlama (ekzoftalmus) şikayetleriyle başvuruyor. Laboratuvar testlerinde T3 ve T4 yüksek, TSH ise ölçülemeyecek kadar DÜŞÜK bulunuyor. Kanda TSH reseptör antikorları (TSI) pozitiftir. Graves Hastalığı tanısı alan bu hastadaki TSH düşüklüğünün temel fizyolojik mekanizması aşağıdakilerden hangisidir?",
        "options": [
            "TSI antikorlarının doğrudan TSH moleküllerini kanda parçalaması",
            "Artmış T3 ve T4 hormonlarının hipofiz ön lobuna uyguladığı güçlü negatif geri bildirim (feedback) etkisi",
            "Hipotalamustan TRH salınımının tümör nedeniyle artması",
            "Tiroid bezinin TSH'yi tüketmesi ve kana vermemesi",
            "İyot eksikliğine bağlı TSH sentez bozukluğu"
        ],
        "answer": "Artmış T3 ve T4 hormonlarının hipofiz ön lobuna uyguladığı güçlü negatif geri bildirim (feedback) etkisi",
        "correctAnswer": 1,
        "explanation": "Graves hastalığında üretilen otoantikorlar (TSI), tiroid bezindeki TSH reseptörlerini uyararak bezden sürekli ve aşırı T3-T4 sentezlettirir (Hipertiroidi). Kanda aşırı artan T3 ve T4, hipotalamus (TRH) ve ön hipofize (TSH) giderek fizyolojik 'negatif feedback' mekanizmasıyla TSH üretimini tamamen baskılar."
    },
    {
        "id": "v9_fiz_2",
        "subject": "Fizyoloji",
        "question": "Bir hastanın elektrokardiyogramında (EKG) DII derivasyonunda R dalgası yüksekliği 15 mm (1.5 mV), DI derivasyonunda ise 5 mm (0.5 mV) olarak ölçülmüştür. Einthoven yasasına göre bu hastanın DIII derivasyonundaki R dalgası yüksekliği teorik olarak kaç mm olmalıdır?",
        "options": [
            "5 mm",
            "10 mm",
            "15 mm",
            "20 mm",
            "-10 mm (negatif)"
        ],
        "answer": "10 mm",
        "correctAnswer": 1,
        "explanation": "Einthoven Yasasına göre, herhangi bir anda üç bipolar ekstremite derivasyonunun potansiyelleri arasında: DI + DIII = DII matematiksel eşitliği vardır. 5 mm + DIII = 15 mm olduğuna göre, DIII = 10 mm olur."
    },
    {
        "id": "v9_fiz_3",
        "subject": "Fizyoloji",
        "question": "Kalp kası (miyokard) hücrelerinin aksiyon potansiyelinde plato fazının (Faz 2) oluşumunu sağlayan ve kalp kasının tetanize olmasını (sürekli kasılı kalıp durmasını) engelleyerek uzun bir mutlak refrakter periyot yaratan asıl iyonik olay nedir?",
        "options": [
            "Hızlı sodyum kanallarının sürekli açık kalması",
            "Potasyumun hücre içine girmesi",
            "L-Tipi yavaş Kalsiyum (Ca++) kanallarından hücre içine kalsiyum girmesi ile potasyum çıkışının dengelenmesi",
            "Hücre içindeki klor iyonlarının dışarı sızması",
            "Na+/K+ pompasının bu fazda durması"
        ],
        "answer": "L-Tipi yavaş Kalsiyum (Ca++) kanallarından hücre içine kalsiyum girmesi ile potasyum çıkışının dengelenmesi",
        "correctAnswer": 2,
        "explanation": "Kalp kası aksiyon potansiyelinde Faz 2 (Plato), L-tipi yavaş kalsiyum kanallarının açılarak hücre içine kalsiyum girmesi ve aynı anda dışarı sızan potasyum akımıyla dengelenmesi sonucu oluşur. Bu uzun plato fazı sayesinde kalp kasının refrakter periyodu uzar ve iskelet kasındaki gibi tetanik kasılma (kramp) oluşması imkansız hale gelir (kalbin pompa işlevi için şarttır)."
    },
    {
        "id": "v9_fiz_4",
        "subject": "Fizyoloji",
        "question": "Akciğer hacimlerinden Rezidüel Volüm (RV) spirometre ile doğrudan ÖLÇÜLEMEZ. Bunun yerine helyum dilüsyon veya vücut pletismografisi yöntemleri kullanılır. Rezidüel volümün tanımı aşağıdakilerden hangisinde doğru verilmiştir?",
        "options": [
            "Sakin solunum sırasında akciğerlere giren hava miktarıdır.",
            "En zorlu bir inspirasyondan sonra çıkarılabilen maksimum hava hacmidir.",
            "Normal ekspirasyon sonrası akciğerlerde kalan toplam havadır (FRC).",
            "En zorlu ve maksimal ekspirasyon çabasından (zorlu nefes vermeden) sonra bile akciğerlerde kalan hava miktarıdır.",
            "Dakikada solunan toplam hava miktarıdır."
        ],
        "answer": "En zorlu ve maksimal ekspirasyon çabasından (zorlu nefes vermeden) sonra bile akciğerlerde kalan hava miktarıdır.",
        "correctAnswer": 3,
        "explanation": "Rezidüel Volüm (Tortu hacim), kişi alabildiği kadar nefes alıp, ardından ZORLU ve sonuna kadar nefes verdikten sonra akciğerlerin kollaps olmasını engelleyen ve içeride kalmaya devam eden hava miktarıdır. Dışarı üflenemediği için klasik spirometre ile ölçülemez."
    },
    {
        "id": "v9_fiz_5",
        "subject": "Fizyoloji",
        "question": "Oksijenin kanda taşınması sırasında, dokulardaki yüksek karbondioksit (CO2) ve düşük pH'nın (yüksek H+), hemoglobinin oksijene afinitesini azaltarak oksijenin dokulara kolayca bırakılmasını sağlamasına ne ad verilir?",
        "options": [
            "Haldane etkisi",
            "Bohr etkisi",
            "Hamburger fenomeni (Klor şifti)",
            "Frank-Starling yasası",
            "Laplace kanunu"
        ],
        "answer": "Bohr etkisi",
        "correctAnswer": 1,
        "explanation": "Kanda CO2 ve H+ konsantrasyonu arttığında (dokularda), oksihemoglobin ayrışma eğrisi sağa kayar ve hemoglobinin O2 bağlama gücü düşerek O2 dokulara serbestleşir. Buna 'Bohr Etkisi' denir. Tam tersi (akciğerlerde O2'nin bağlanmasıyla CO2'nin atılması) ise Haldane etkisidir."
    },
    {
        "id": "v9_fiz_6",
        "subject": "Fizyoloji",
        "question": "Sindirim sisteminde mideden salgılanan İntrinsik Faktör (Intrinsic Factor - IF), hangi vitaminin ince bağırsaktan (terminal ileum) emilebilmesi için mutlak gereklidir ve eksikliğinde Pernisiyöz anemi tablosu görülür?",
        "options": [
            "Vitamin C (Askorbik asit)",
            "Vitamin B1 (Tiamin)",
            "Vitamin B12 (Kobalamin)",
            "Vitamin K",
            "Folik asit (B9)"
        ],
        "answer": "Vitamin B12 (Kobalamin)",
        "correctAnswer": 2,
        "explanation": "Midenin paryetal hücrelerinden asitle birlikte İntrinsik Faktör (IF) salgılanır. Diyetle alınan B12 vitamini IF'ye bağlanmadan terminal ileumdaki reseptörlerce tanınamaz ve emilemez. Midesi alınan veya paryetal hücreleri otoimmün harabiyete uğrayanlarda B12 eksikliğine bağlı makrositik pernisiyöz anemi gelişir."
    },
    {
        "id": "v9_fiz_7",
        "subject": "Fizyoloji",
        "question": "Bir hastanın böbrek fonksiyonları (Glomerüler Filtrasyon Hızı - GFR) hesaplanırken klinikte altın standart olarak Kreatinin Klerensi kullanılmaktadır. GFR ölçümü için ideal bir maddenin taşıması gereken MÜKEMMEL özellik aşağıdakilerden hangisidir?",
        "options": [
            "Glomerüllerden serbestçe süzülmesi, tübüllerde tamamının geri emilmesi",
            "Glomerüllerden hiç süzülmeyip sadece tübüllerden aktif olarak salgılanması",
            "Glomerüllerden serbestçe süzülmesi, tübüllerde ne emilmesi ne de salgılanması (İnülin gibi)",
            "Plazma proteinlerine sıkıca bağlı olması",
            "Böbrek tübüllerinde sentezlenip idrara bırakılması"
        ],
        "answer": "Glomerüllerden serbestçe süzülmesi, tübüllerde ne emilmesi ne de salgılanması (İnülin gibi)",
        "correctAnswer": 2,
        "explanation": "İdeal bir GFR (Glomerüler Filtrasyon Hızı) belirtecinin, sadece glomerülden süzülmesi (filtrasyona uğraması), tübüllerden geçerken hiçbir şekilde reabsorbe (geri emilim) veya sekrete (salgılanma) olmaması ve metabolize edilmemesi gerekir. Bu özellikleri tam olarak taşıyan bitkisel madde 'İnülin'dir. Klinik pratikte ise buna en yakın olan endojen madde olan Kreatinin kullanılır (Kreatinin tübüllerden çok az miktarda sekrete edildiği için GFR'yi gerçekte olduğundan %10-20 fazla gösterir)."
    },
    {
        "id": "v9_fiz_8",
        "subject": "Fizyoloji",
        "question": "Böbreklerde asit-baz dengesinin sağlanmasında en fazla idrar tamponu görevi gören ve böylece çok büyük miktarlarda H+ iyonunun pH sınırını (4.5) aşmadan idrarla atılabilmesini sağlayan sistemler hangi seçenekte doğru verilmiştir?",
        "options": [
            "Bikarbonat ve Sülfat",
            "Fosfat ve Amonyak (NH3 / NH4+)",
            "Laktat ve Klorür",
            "Sodyum ve Potasyum",
            "Karbonik anhidraz enzimi"
        ],
        "answer": "Fosfat ve Amonyak (NH3 / NH4+)",
        "correctAnswer": 1,
        "explanation": "İdrarın pH'sı en fazla 4.5'e düşebilir. Serbest H+ atılımı kapasitesi bu yüzden çok düşüktür. Atılan H+ iyonları tübül lümeninde tamponlanmalıdır. Bu tamponlamayı esas olarak Titre Edilebilir Asitleri oluşturan Fosfat (HPO4-- -> H2PO4-) tampon sistemi ve glutamin metabolizmasıyla sentezlenen Amonyak (NH3 -> NH4+) tampon sistemi gerçekleştirir."
    },
    {
        "id": "v9_fiz_9",
        "subject": "Fizyoloji",
        "question": "Safra kesesinin fizyolojik görevleriyle ilgili aşağıdaki ifadelerden hangisi yanlıştır?",
        "options": [
            "Karaciğerin ürettiği safrayı depolar.",
            "Safranın içindeki suyu ve sodyumu aktif olarak emerek safrayı 5-20 kat konsantre hale getirir.",
            "Yağlı gıdaların alımından sonra CCK (Kolesistokinin) hormonu uyarısıyla kasılarak safrayı onikiparmak bağırsağına boşaltır.",
            "Safra tuzlarını (kolik asit vb.) kolesterolden bizzat kendisi sentezler.",
            "Açlık durumunda Oddi sfinkterinin kapalı olması sayesinde safra, safra kesesinde birikir."
        ],
        "answer": "Safra tuzlarını (kolik asit vb.) kolesterolden bizzat kendisi sentezler.",
        "correctAnswer": 3,
        "explanation": "Safra asitleri/tuzları safra kesesinde değil, KARACİĞERDEKİ hepatositlerde kolesterolden sentezlenir. Safra kesesinin (Vesica biliaris) görevi sadece safrayı depolamak, yoğunlaştırmak (konsantre etmek) ve CCK uyarısıyla katarak bağırsağa salgılamaktır; herhangi bir madde sentezlemez."
    },
    {
        "id": "v9_fiz_10",
        "subject": "Fizyoloji",
        "question": "Adet döngüsünde (Menstrüel siklus) ovulasyonu (yumurtlamayı) tetikleyen, siklusun ortasında (14. gün civarı) pik (zirve) yaparak folikülün çatlamasını sağlayan hormon aşağıdakilerden hangisidir?",
        "options": [
            "FSH (Folikül Stimüle Edici Hormon)",
            "Östrojen",
            "Progesteron",
            "LH (Lüteinleştirici Hormon)",
            "Prolaktin"
        ],
        "answer": "LH (Lüteinleştirici Hormon)",
        "correctAnswer": 3,
        "explanation": "Ovulasyondan yaklaşık 1-2 gün önce kanda östrojen seviyesinin kritik bir eşiğe ulaşıp uzun süre yüksek kalması, pozitif feedback yaparak hipofizden devasa bir LH salgılanmasına ('LH piki') neden olur. LH piki ovulasyonun fizyolojik tetikleyicisidir."
    },
    {
        "id": "v9_fiz_11",
        "subject": "Fizyoloji",
        "question": "Erkek üreme sisteminde spermatogenezi (sperm üretimini) doğrudan uyaran Sertoli hücreleri üzerinde reseptörü bulunan hormon ile, testosteron sentezini uyaran Leydig hücreleri üzerinde reseptörü bulunan hormon sırasıyla aşağıdakilerden hangisidir?",
        "options": [
            "FSH - LH",
            "LH - FSH",
            "Testosteron - İnhibin",
            "GnRH - FSH",
            "Prolaktin - Oksitosin"
        ],
        "answer": "FSH - LH",
        "correctAnswer": 0,
        "explanation": "Spermatogenezi destekleyen ve besleyen Sertoli hücreleri FSH (Folikül Stimüle Edici Hormon) ile uyarılır. Testosteron üreten Leydig hücreleri (İnterstisyel hücreler) ise LH (Lüteinleştirici Hormon) ile uyarılır."
    },
    {
        "id": "v9_fiz_12",
        "subject": "Fizyoloji",
        "question": "Gebelik sırasında korpus luteumun bozulmasını engelleyerek progesteron ve östrojen salgısının devamını sağlayan, embriyonun sinsityotrofoblastlarından salgılanan ve gebelik testlerinde (idrar/kan) pozitif sonuç veren hormon hangisidir?",
        "options": [
            "İnsan Koryonik Somatomammotropini (hCS)",
            "Oksitosin",
            "İnsan Koryonik Gonadotropini (hCG)",
            "Relaksin",
            "Progesteron"
        ],
        "answer": "İnsan Koryonik Gonadotropini (hCG)",
        "correctAnswer": 2,
        "explanation": "İnsan Koryonik Gonadotropini (hCG), implantasyondan hemen sonra trofoblastlar tarafından üretilir. LH'ye benzer etki göstererek Korpus Luteumu ayakta tutar. Plasenta kendi östrojen/progesteronunu üretebilecek kapasiteye gelene kadar (ilk trimester) düşüğü engeller. İdrar gebelik testlerinde bakılan maddedir."
    },
    {
        "id": "v9_fiz_13",
        "subject": "Fizyoloji",
        "question": "Somatik büyüme hormonu (GH / Somatotropin), özellikle kemik ve kıkırdakların uzamasını (büyümesini) doğrudan kendisi uyarmaktan ziyade, karaciğerde hangi maddenin sentezini artırarak etkisini gösterir?",
        "options": [
            "Kortizol",
            "IGF-1 (İnsülin benzeri büyüme faktörü-1 / Somatomedin C)",
            "Tiroksin (T4)",
            "Glukagon",
            "Kalsitonin"
        ],
        "answer": "IGF-1 (İnsülin benzeri büyüme faktörü-1 / Somatomedin C)",
        "correctAnswer": 1,
        "explanation": "Büyüme hormonu (GH), karaciğere giderek Somatomedinleri (en önemlisi IGF-1) sentezlettirir. Büyümenin büyük kısmı (özellikle kıkırdak ve kemikteki kondrosit proliferasyonu) doğrudan GH'nin değil, IGF-1'in etkisiyle gerçekleşir."
    },
    {
        "id": "v9_fiz_14",
        "subject": "Fizyoloji",
        "question": "Kan şekeri yükseldiğinde pankreas beta hücrelerinden insülin salınımı iki fazlı (bifazik) olarak gerçekleşir. İlk fazın (hızlı ve kısa) mekanizması ile ikinci fazın (yavaş ve uzun) mekanizması arasındaki fizyolojik fark nedir?",
        "options": [
            "İlk faz karaciğerden salınır, ikinci faz pankreastan salınır.",
            "İlk faz glukagona yanıt, ikinci faz somatostatine yanıttır.",
            "İlk faz önceden sentezlenip veziküllerde depolanmış hazır insülinin aniden boşalmasıdır; ikinci faz ise yeni insülin sentezlenip salgılanmasıdır.",
            "İlk faz kan kalsiyumuna bağlıdır, ikinci faz klor iyonlarına bağlıdır.",
            "İlk fazda inaktif proinsülin, ikinci fazda C-peptid salgılanır."
        ],
        "answer": "İlk faz önceden sentezlenip veziküllerde depolanmış hazır insülinin aniden boşalmasıdır; ikinci faz ise yeni insülin sentezlenip salgılanmasıdır.",
        "correctAnswer": 2,
        "explanation": "İnsülin sekresyonunun birinci fazı (glukoz yükseldikten sonraki ilk 3-5 dakika), hücre içinde hazır halde bekleyen depo granüllerinin ekzositozla kana karışmasıdır. İkinci faz (dakikalar ve saatler süren) ise yeni insülin mRNA transkripsiyonu ve translasyonu ile üretilen taze insülinin salgılanmasıdır."
    },
    {
        "id": "v9_fiz_15",
        "subject": "Fizyoloji",
        "question": "Stres durumunda CRH-ACTH ekseni uyarılmasıyla adrenal korteksten sentezlenen kortizol (glukokortikoidler), kan şekerini yükseltmek için temel olarak karaciğerde hangi mekanizmayı kullanır?",
        "options": [
            "Glikolizi hızlandırır.",
            "Glikojen depolanmasını engeller.",
            "Glukoneogenezi (amino asitlerden glukoz yapımını) uyarır.",
            "İnsülin salınımını artırır.",
            "Böbrekten glukoz atılımını azaltır."
        ],
        "answer": "Glukoneogenezi (amino asitlerden glukoz yapımını) uyarır.",
        "correctAnswer": 2,
        "explanation": "Kortizol, ekstremitelerdeki iskelet kaslarından proteinleri yıkarak amino asitleri karaciğere taşır ve karaciğerde bu amino asitlerden glukoneogenez (yeni glukoz sentezi) ile kan şekerini şiddetle artırır. Ayrıca hücrelerin glukoz kullanımını da azaltır."
    },
    {
        "id": "v9_fiz_16",
        "subject": "Fizyoloji",
        "question": "Kan basıncının kısa süreli (saniyeler içinde) düzenlenmesinde en önemli kontrol mekanizması hangisidir?",
        "options": [
            "Böbrekler tarafından sodyum-su atılımının değiştirilmesi",
            "Renin-Anjiyotensin-Aldosteron sistemi (RAAS)",
            "Baroreseptör Refleks mekanizması",
            "Eritropoietin salgısı",
            "ADH salınımının artması"
        ],
        "answer": "Baroreseptör Refleks mekanizması",
        "correctAnswer": 2,
        "explanation": "Kan basıncındaki ani değişiklikleri (örneğin yatar pozisyondan ayağa kalkınca oluşan düşmeyi) saniyeler içinde otonom sinir sistemi vasıtasıyla düzelten sistem Baroreseptör refleksidir (Karotis sinüsü ve aort kavsi). Böbreklerin su/tuz dengesi (RAAS vb.) ise uzun süreli (günler/haftalar) kan basıncı kontrolünü sağlar."
    },
    {
        "id": "v9_fiz_17",
        "subject": "Fizyoloji",
        "question": "Kanda kalsiyum seviyesi düştüğünde (hipokalsemi), Paratiroid Hormon (PTH) seviyesi artar. PTH, kan kalsiyumunu normale döndürmek için aşağıdaki etkilerden hangisini YAPMAZ?",
        "options": [
            "Kemiklerde osteoklast aktivitesini artırarak kemik rezorpsiyonunu hızlandırır (Kalsiyumu kana geçirir).",
            "Böbreklerde proksimal tübülde fosfat geri emilimini İNHİBE eder (Fosfatüri yapar).",
            "Böbreklerde distal tübülde kalsiyum geri emilimini artırır.",
            "Böbreklerde 1-alfa-hidroksilaz enzimini aktive ederek aktif D vitamini (1,25-dihidroksikolekalsiferol) yapımını artırır.",
            "İnce bağırsaktan kalsiyum emilimini DOĞRUDAN (D vitamininden bağımsız) artırır."
        ],
        "answer": "İnce bağırsaktan kalsiyum emilimini DOĞRUDAN (D vitamininden bağımsız) artırır.",
        "correctAnswer": 4,
        "explanation": "PTH'nin bağırsaklardan kalsiyum emilimi üzerine DOĞRUDAN bir etkisi yoktur. Bu etkiyi böbrekte aktiflediği D vitamini üzerinden DOLAYLI olarak yapar. Diğer seçeneklerdeki kemik rezorpsiyonu, renal Ca emilimi ve renal fosfat atılımı PTH'nin doğrudan etkileridir."
    },
    {
        "id": "v9_fiz_18",
        "subject": "Fizyoloji",
        "question": "Gözde uzağa bakarken yakındaki bir nesneye bakıldığında (Akodomasyon Refleksi) net görüntünün retineye düşebilmesi için gözde hangi üçlü fizyolojik değişiklik aynı anda meydana gelir?",
        "options": [
            "Miyozis (göz bebeği küçülür), Gözler konverje olur (içe döner), Lens kırıcılığı azalır (lens yassılaşır)",
            "Midriyazis (göz bebeği büyür), Gözler diverje olur (dışa döner), Lens bombeliği artar",
            "Miyozis, Gözlerde konverjans, Lensin bombeliği ve kırıcılığı ARTAR",
            "Midriyazis, Gözlerde konverjans, Lens yassılaşır",
            "Göz bebeği değişmez, Gözler içe döner, Lens bombeliği artar"
        ],
        "answer": "Miyozis, Gözlerde konverjans, Lensin bombeliği ve kırıcılığı ARTAR",
        "correctAnswer": 2,
        "explanation": "Yakına bakma (Akomodasyon) refleksinin üç bileşeni vardır: 1. Siliyer kaslar kasılır ve lens kalınlaşıp bombeleşerek (kırıcılığı artarak) ışığı daha güçlü odaklar. 2. Pupil sfinkter kası kasılır ve ışığın sadece lensin merkezinden geçmesi için göz bebeği küçülür (miyozis). 3. İki göz küresi içe doğru döner (konverjans)."
    },
    {
        "id": "v9_fiz_19",
        "subject": "Fizyoloji",
        "question": "İşitme fizyolojisinde, iç kulaktaki Corti organında bulunan ve ses dalgalarının mekanik enerjisini (titreşimlerini) elektriksel aksiyon potansiyellerine dönüştüren (transdüksiyon) asıl reseptör hücreler hangileridir?",
        "options": [
            "Destek hücreleri",
            "İç titrek (tüylü) hücreler (Inner hair cells)",
            "Dış titrek (tüylü) hücreler (Outer hair cells)",
            "Reissner zarı hücreleri",
            "Stria vascularis hücreleri"
        ],
        "answer": "İç titrek (tüylü) hücreler (Inner hair cells)",
        "correctAnswer": 1,
        "explanation": "Dış tüylü hücreler daha çok ses amplifikasyonunda (yükseltici/motor) görevliyken, asıl duyu reseptörleri ve işitme sinirine (N. cochlearis) %90-95 oranında afferent impuls gönderen hücreler İÇ tüylü hücrelerdir (Inner hair cells)."
    },
    {
        "id": "v9_fiz_20",
        "subject": "Fizyoloji",
        "question": "Denge fizyolojisinde, yarım daire kanallarındaki (canales semicirculares) ampulla içinde yer alan ve başın rotasyonel (açısal) hareketlerini algılayan yapı aşağıdakilerden hangisidir?",
        "options": [
            "Macula sacculi",
            "Macula utriculi",
            "Crista ampullaris",
            "Corti organı",
            "Otilitik membran"
        ],
        "answer": "Crista ampullaris",
        "correctAnswer": 2,
        "explanation": "Yarım daire kanallarının genişlemiş uçlarında (ampulla) bulunan reseptör alana 'Crista ampullaris' denir. Üzerindeki cupula tabakası, baş sağa-sola döndüğünde (rotasyonel ivmelenme) içindeki endolenf sıvısının hareketiyle eğilir ve denge sinirini uyarır."
    },
    {
        "id": "v9_fiz_21",
        "subject": "Fizyoloji",
        "question": "Omuriliğin inen motor yollarından hangisi, özellikle dik duruşu (postürü) sağlamak için ekstansör kasların tonusunu artırıcı (uyarıcı) etkiye sahiptir?",
        "options": [
            "Tractus corticospinalis lateralis",
            "Tractus rubrospinalis",
            "Tractus vestibulospinalis",
            "Tractus spinothalamicus",
            "Fasciculus cuneatus"
        ],
        "answer": "Tractus vestibulospinalis",
        "correctAnswer": 2,
        "explanation": "Tractus vestibulospinalis (ve tr. reticulospinalis pontinus), yerçekimine karşı postürü korumak için aksiyal (gövde) ve ekstansör kasları güçlü bir şekilde kolaylaştırır (eksite eder). Tractus rubrospinalis ise tam tersine fleksör kasları uyarır."
    },
    {
        "id": "v9_fiz_22",
        "subject": "Fizyoloji",
        "question": "Bir kaza sonucu beyincik (serebellum) hasarı alan bir hastada aşağıdaki nörolojik bulgulardan hangisinin GÖRÜLMESİ BEKLENMEZ?",
        "options": [
            "Ataksi (Dengesiz, sarhoşvari yürüyüş)",
            "Disdiadokokinezi (Hızlı ardışık hareketleri yapamama)",
            "İntansiyonel tremor (İstemli bir hedefe uzanırken ortaya çıkan titreme)",
            "Hemiplaji (Vücudun bir yarısında tam felç / paralizi)",
            "Nistagmus (Gözlerde istemsiz sıçrama hareketleri)"
        ],
        "answer": "Hemiplaji (Vücudun bir yarısında tam felç / paralizi)",
        "correctAnswer": 3,
        "explanation": "Serebellum motor hareketleri başlatmaz, onları koordine eder. Bu nedenle serebellum hasarında felç (kuvvet kaybı/paralizi) GÖRÜLMEZ. Onun yerine hareketlerin koordinasyonu bozulur (ataksi, dismetri, disdiadokokinezi, intansiyonel tremor)."
    },
    {
        "id": "v9_fiz_23",
        "subject": "Fizyoloji",
        "question": "İskelet kasının kasılması (kayan flamanlar teorisi) sırasında, ince flamanlar (aktin) üzerindeki miyozin bağlanma bölgelerini kapatarak kasın istirahat halinde gevşek kalmasını sağlayan protein hangisidir?",
        "options": [
            "Troponin C",
            "Tropomiyozin",
            "Titin",
            "Nebulin",
            "Miyozin başı"
        ],
        "answer": "Tropomiyozin",
        "correctAnswer": 1,
        "explanation": "İstirahat halindeki kasta, uzun ipliksi bir protein olan Tropomiyozin, aktin sarmalı üzerindeki aktif bölgeleri (miyozinin bağlanacağı yerleri) kapatır. Kasılma için hücre içine giren kalsiyum Troponin C'ye bağlanarak şekil değiştirir ve Tropomiyozini bu aktif bölgelerin üzerinden kaydırır."
    },
    {
        "id": "v9_fiz_24",
        "subject": "Fizyoloji",
        "question": "Sinir kas kavşağında (nöromusküler kavşak), motor nörondan salgılanan asetilkolin, kas hücresi zarındaki (motor son plak) hangi tip reseptöre bağlanarak kasta aksiyon potansiyelini başlatır?",
        "options": [
            "Muskarinik (M3) reseptörler",
            "Nikotinik (Nm) kolinerjik reseptörler",
            "Alfa-1 adrenerjik reseptörler",
            "Beta-2 adrenerjik reseptörler",
            "GABA-A reseptörleri"
        ],
        "answer": "Nikotinik (Nm) kolinerjik reseptörler",
        "correctAnswer": 1,
        "explanation": "Çizgili iskelet kaslarındaki motor son plakta Nikotinik tip (kas tipi - Nm) kolinerjik reseptörler bulunur. Bunlar ligand kapılı iyon (Na+) kanallarıdır. Kalp ve düz kaslarda ise Muskarinik kolinerjik reseptörler görev yapar."
    },
    {
        "id": "v9_fiz_25",
        "subject": "Fizyoloji",
        "question": "Aksiyon potansiyelinin miyelinli sinir liflerindeki iletimiyle ilgili olarak aşağıdakilerden hangisi yanlıştır?",
        "options": [
            "İletim Ranvier boğumlarından (düğümlerinden) atlayarak ilerler (Saltatorik iletim).",
            "Miyelin kılıfı, zarın kapasitansını azaltarak impulsun hızlı yayılmasını sağlar.",
            "Ranvier boğumlarında voltaj kapılı sodyum kanalları çok yoğun olarak bulunur.",
            "Miyelinsiz liflere göre çok daha hızlıdır ve enerji tasarrufu (daha az Na+/K+ ATPaz çalışması) sağlar.",
            "İletim hızı sinir lifinin çapı küçüldükçe artar."
        ],
        "answer": "İletim hızı sinir lifinin çapı küçüldükçe artar.",
        "correctAnswer": 4,
        "explanation": "Sinir lifinde iletim hızı, lifin çapı ile DOĞRU orantılıdır. Çap ne kadar büyükse (ve miyelin ne kadar kalınsa) iç direnç azalacağından iletim hızı o kadar fazla olur. Örneğin A-alfa motor lifleri en kalın ve en hızlı olanlardır."
    },
    {
        "id": "v9_fiz_26",
        "subject": "Fizyoloji",
        "question": "Kan pıhtılaşması (koagülasyon) kaskadında, Hemofili A hastalığına neden olan ve Eksikliği PTT (Parsiyel Tromboplastin Zamanı) testinde uzamaya yol açan koagülasyon faktörü hangisidir?",
        "options": [
            "Faktör II (Protrombin)",
            "Faktör VII",
            "Faktör VIII (Antihemofilik Faktör A)",
            "Faktör IX",
            "Faktör X"
        ],
        "answer": "Faktör VIII (Antihemofilik Faktör A)",
        "correctAnswer": 2,
        "explanation": "Hemofili A, X kromozomuna bağlı resesif geçen ve Faktör VIII (Antihemofilik Faktör A) eksikliğinden kaynaklanan kanama hastalığıdır. İntrensek yolu etkilediği için PTT (aPTT) testi uzar. Faktör IX eksikliği ise Hemofili B'ye yol açar."
    },
    {
        "id": "v9_fiz_27",
        "subject": "Fizyoloji",
        "question": "Eritropoietin (EPO) hormonunun birincil görevi kırmızı kan hücresi (eritrosit) üretimini artırmaktır. Bu hormon yetişkinlerde ağırlıklı olarak (%90) hangi organ tarafından ve hangi uyarana yanıt olarak sentezlenir?",
        "options": [
            "Karaciğer / Yüksek kan glukozu",
            "Kemik iliği / Düşük demir seviyesi",
            "Dalak / Düşük kan basıncı",
            "Böbrekler / Doku hipoksisi (Oksijen azlığı)",
            "Akciğerler / Yüksek karbondioksit"
        ],
        "answer": "Böbrekler / Doku hipoksisi (Oksijen azlığı)",
        "correctAnswer": 3,
        "explanation": "Eritropoietin (EPO), temel olarak böbreklerdeki tübülointerstisyel hücreler tarafından üretilir. Üretimi tetikleyen asıl sinyal dokuya giden oksijen miktarının azalmasıdır (Hipoksi). Böbrek yetmezliği olan hastalarda bu yüzden ciddi anemi gelişir."
    },
    {
        "id": "v9_fiz_28",
        "subject": "Fizyoloji",
        "question": "Gözdeki retina tabakasında bulunan fotoreseptör hücrelerden Çubukların (Rods) ve Konilerin (Cones) görevleriyle ilgili aşağıdakilerden hangisi yanlıştır?",
        "options": [
            "Çubuklar düşük ışıkta (karanlık/skotopik) görmeden sorumludur.",
            "Koniler aydınlıkta renkli görmeden (fotopik) sorumludur.",
            "Fovea centralis (Sarı lekenin merkezi) bölgesinde sadece çubuklar yoğun olarak bulunur.",
            "Çubuklar siyah-beyaz (gri tonları) algılar, renk ayrıntısı veremezler.",
            "Koniler yüksek çözünürlüklü (keskin) görmeyi sağlarlar."
        ],
        "answer": "Fovea centralis (Sarı lekenin merkezi) bölgesinde sadece çubuklar yoğun olarak bulunur.",
        "correctAnswer": 2,
        "explanation": "Fovea centralis, retinanın en keskin görme alanıdır ve burada çubuk hücresi (Rods) HİÇ BULUNMAZ. Sadece sıkışık halde Koniler (Cones) bulunur. Çubuklar retinanın periferik (çevresel) kısımlarında çok yoğun bulunur."
    },
    {
        "id": "v9_fiz_29",
        "subject": "Fizyoloji",
        "question": "Ağrı duyusunun iletilmesinde, deriden kalkan yavaş, sızlayıcı (kronik) ağrı duyusunu omuriliğe taşıyan, miyelinsiz ve iletim hızı en yavaş olan sinir lifi tipi hangisidir?",
        "options": [
            "A-alfa lifleri",
            "A-beta lifleri",
            "A-delta lifleri",
            "B lifleri",
            "C lifleri"
        ],
        "answer": "C lifleri",
        "correctAnswer": 4,
        "explanation": "Ağrı duyusu iki tip lifle taşınır: 1. Hızlı, keskin, batıcı ağrı (A-delta lifleri - miyelinli, hızlı). 2. Yavaş, künt, sızlayıcı, yanan tarzda ağrı (C lifleri - miyelinsiz, en yavaş)."
    }
];

window.examsData['tam_kapsamli_final_v9'].questions.push(...v9Part1);

